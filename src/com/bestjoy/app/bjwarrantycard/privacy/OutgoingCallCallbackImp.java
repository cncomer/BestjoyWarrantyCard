...
public class OutgoingCallCallbackImp extends MonitorCallbackImp{
	private static OutgoingCallCallbackImp mInstance = new OutgoingCallCallbackImp();
	...
	private OutgoingCallCallbackImp(){}
...
//		String phoneNum = Contents.MingDang.buildValidPhoneNumber(number, MyApplication.getInstance().getPreferAreaCode());
	...
	public void onPhoneOffhook(boolean outgoing, String number) {
		...
		//当对方接起电话，我们也需要隐藏去电提示框
		if (outgoing) hideCallFlowDialog();
	}

	@Override
	public boolean isEnabled(SharedPreferences prefers) {
		return prefers.getBoolean(SettingsPreferenceActivity.KEY_PRIVACY_OUTGOING_CALL, true);
	}
