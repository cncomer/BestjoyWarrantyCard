...
import com.bestjoy.app.warrantycard.service.EmptyService;
	...
	public static abstract class VcfAsyncDownloadHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case VcfAsyncDownloadTask.EVENT_SAVE_CONTACT_FINISH: {
				...
				break;
			case VcfAsyncDownloadTask.EVENT_SAVE_MERCHANT_CONTACT_FINISH: {
				Bundle data = msg.getData();
				long id = data.getLong(Intents.EXTRA_ID, -1);
				if (id > 0) {
					Uri uri = data.getParcelable(Intents.EXTRA_URI);
					String mm = data.getString(Intents.EXTRA_BID);
					MyApplication.getInstance().showMessage(R.string.msg_save_mylife_ok);
					if (!onSaveFinished(uri, mm)) {
						//这里要做显示商家名片的动作
						if (MyAccountManager.getInstance().hasLoginned()) {
							//我们有账户，我们需要跳转
							Context context = MyApplication.getInstance();
							Intent intent = MyLifeListItemActivity.createIntent(context, id);
							intent.addFlags(Intent.FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS);
							context.startActivity(intent);
						}
					}
				} else if (id == -1) {
					MyApplication.getInstance().showMessage(R.string.msg_save_mylife_failed);
				}
			}
				break;
		 ...
		 */
		private void internalSaveMerchantContactAfterDownload(AddressBookParsedResult mAddressBookParsedResult) {
			long id = saveMerchantConatctLocked(mAddressBookParsedResult);
			Message message = Message.obtain(VcfAsyncDownloadHandler.this);
			message.what = VcfAsyncDownloadTask.EVENT_SAVE_MERCHANT_CONTACT_FINISH;
			Bundle data = new Bundle();
			data.putLong(Intents.EXTRA_ID, id);
			if (id > 0) {
				data.putParcelable(Intents.EXTRA_URI, ContentUris.withAppendedId(BjnoteContent.MyLife.CONTENT_URI, id));
			}
